...
namespace LigaBA.ClasesLigaBA
{
    public class FixtureViejo
    {
        ...
        private List<int> toleranciasAcumuladas;
        ...
        public FixtureViejo(List<Fecha> f)
        {
            this.fixture = f;
        }

        public FixtureViejo(List<Institucion> equipos)
        {
        ...
        private void inicializar()
        {
            ...
            toleranciasAcumuladas = new List<int>();
            ...
            if (equipos.Count > 1)
            {
                ...
                for (int i = 1; i <= cantidadDeFechas; i++)
                {
                    ...
                    for (int j = 1; j <= cantidadDePartidosSimultaneos; j++)
                    {
                        ...
                        {
                            //Fue VISITANTE
                            if ((fixture[i - 1].get_visitante()).get_id() == visitante.get_id() && !toleranciaDeEstado(visitante))
                            {
                                tolerancia[equipos.IndexOf(visitante)]--;
                            }
                            //Fue LOCAL
                            if ((fixture[i - 1].get_local()).get_id() == local.get_id() && !toleranciaDeEstado(local))
                            {
                                tolerancia[equipos.IndexOf(local)]--;
                            }
                        }
        ...
        private void reiniciarTolerancia(int[] tolerancia)
        {
            ...
            while (contador < (cantidadEquipos / 2))
            {
                ...
                while (toleranciasAcumuladas.Contains(i)) { i = random.Next(0, cantidadEquipos - 1); }
                
                if (tolerancia[i] == 0)
                {
                    tolerancia[i] = 1;
                    toleranciasAcumuladas.Add(i);
                    contador++;
                }
            }
        }
